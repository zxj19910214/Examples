...
for x_cr = 0.01:0.001:0.97
    for x_al = 0.01:0.001:0.08
        for x_fe = 0.01:0.001:0.5
            ...
            tc_check_error;
            tc_set_phase_status('BCC_A2','entered',1);
            tc_check_error;
            tc_set_phase_status('BCC_A2#2','suspended',0);
            tc_check_error;
            tc_poly3_command('s-a-s f');
            tc_compute_equilibrium;    
            tc_check_error;
% get the Gibbs energy
G_Bcc = tc_get_value('GM(BCC_A2)') -8.314*973*(x_cr*log(x_cr)+x_al*log(x_al)+...
